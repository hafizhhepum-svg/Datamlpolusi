#include <iostream>
#include <fstream>
#include <vector>
#include <cmath>
#include <algorithm>

#include "TCanvas.h"
#include "TGraph.h"
#include "TF1.h"
#include "TAxis.h"
#include "TStyle.h"
#include "TPaveText.h"
#include "TLegend.h"

void grafik3() {

    gStyle->SetOptStat(0);
    gStyle->SetOptFit(0);

    std::ifstream infile("out_67.txt");

    if (!infile.is_open()) {
        std::cout << "Error opening file!" << std::endl;
        return;
    }

    std::vector<double> x_vals;
    std::vector<double> y_vals;

    double x,y;

    while (infile >> x >> y) {
        if (x==0 || y==0) continue;
        x_vals.push_back(x);
        y_vals.push_back(y);
    }

    infile.close();

    int n = x_vals.size();

    if (n < 2) {
        std::cout << "Not enough valid data!" << std::endl;
        return;
    }

    TCanvas *c1 = new TCanvas("c1","Scatter Plot PM2.5",1200,1000);

    c1->SetLeftMargin(0.13);
    c1->SetBottomMargin(0.12);

    TGraph *gr = new TGraph(n,&x_vals[0],&y_vals[0]);

    gr->SetTitle(";NO_{2} Observed;NO_{2} Predicted");

    // marker open circle
    gr->SetMarkerStyle(24);
    gr->SetMarkerSize(0.7);
    gr->SetMarkerColor(kBlue);

    gr->Draw("AP");

    // set range axis
    gr->GetXaxis()->SetRangeUser(0,25);
    gr->GetYaxis()->SetRangeUser(0,25);

    // axis styling
    gr->GetXaxis()->CenterTitle();
    gr->GetYaxis()->CenterTitle();

    gr->GetXaxis()->SetTitleSize(0.05);
    gr->GetYaxis()->SetTitleSize(0.05);

    gr->GetXaxis()->SetTitleOffset(0.9);
    gr->GetYaxis()->SetTitleOffset(1.0);

    // LINEAR FIT
    TF1 *fit = new TF1("fit","pol1",0,25);

    gr->Fit(fit,"R");

    fit->SetLineColor(kRed);
    fit->SetLineWidth(2);

    double c = fit->GetParameter(0);
    double m = fit->GetParameter(1);

    // hitung R2
    double y_mean = 0;

    for(int i=0;i<n;i++)
        y_mean += y_vals[i];

    y_mean /= n;

    double SS_tot = 0;
    double SS_res = 0;

    for(int i=0;i<n;i++){

        double y_fit = fit->Eval(x_vals[i]);

        SS_tot += pow(y_vals[i] - y_mean,2);
        SS_res += pow(y_vals[i] - y_fit,2);

    }

    double R2 = 1 - (SS_res / SS_tot);

    // -----------------------
    // LEGEND KIRI ATAS
    // -----------------------

    TLegend *leg = new TLegend(0.15,0.78,0.45,0.88);

    leg->AddEntry(gr,"Data","p");
    leg->AddEntry(fit,Form("Fit line (slope = %.3f)",m),"l");

    leg->SetBorderSize(0);
    leg->SetFillStyle(0);
    leg->SetTextSize(0.035);

    leg->Draw();

    // -----------------------
    // R2 KANAN ATAS
    // -----------------------

    TPaveText *pt = new TPaveText(0.75,0.85,0.9,0.92,"NDC");

    pt->AddText(Form("R^{2} = %.3f",R2));

    pt->SetFillColor(0);
    pt->SetBorderSize(0);
    pt->SetTextSize(0.04);

    pt->Draw();

    c1->SaveAs("scatter_PM25_fit.png");
}
